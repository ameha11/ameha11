...
gender<-data %>% select(sex) %>% group_by(sex) %>% 
  ...
  hc_title(text = 'Interview by gender', style = list(fontSize = '12px', fontWeight = 'bold')) 
gender
```
...
location <-data %>% select(location) %>% group_by(location) %>% 
  summarise(count = n()) %>% drop_na() %>% 
  hchart('pie', hcaes(x = location, y = count, color='location')) %>% 
  hc_tooltip(pointFormat='<b>Proportion: </b> {point.percentage:,.2f}%') %>% 
  hc_title(text = 'Interview by location', style = list(fontSize = '12px', fontWeight = 'bold')) 
location
```
...
woreda_interview <-hh_data %>% select(Woreda) %>% group_by(Woreda) %>% 
  ...
  hc_title(text = 'NO OF INTERVIEW BY WOREDA', style = list(fontSize = '12px', fontWeight = 'bold')) 
woreda_interview
...
Column
-----------------------------------------------------------------------
### HOUSEHOLD (EXPECTED vs ACTUAL) {data-width=600}
...
```{r}
Ebinat_kebeles<- hh_data %>% dplyr::select('Woreda', 'Kebele') %>% drop_na()%>%  dplyr::filter(Woreda=="Ebinat")%>% dplyr::group_by(Kebele) %>% dplyr::summarise(No_of_kebele= n()) %>% dplyr::arrange(No_of_kebele)

Ebinat_kebeles$Kebele<- factor(Ebinat_kebeles$Kebele, levels=Ebinat_kebeles[["Kebele"]])

#plot fig 4

fig5 <- Ebinat_kebeles %>% plot_ly()
fig5 <- fig5 %>% add_trace(x = ~Kebele, y = ~No_of_kebele, type = 'bar',  name = '', marker = list(color = '#2B3A55'))
...
### DEWA CHEFA WOREDA BY KEBELE {data-width=500}
```{r}
Dewat_kebeles<- hh_data %>% dplyr::select('Woreda', 'Kebele') %>% drop_na()%>%  dplyr::filter(Woreda=="Dewa Chefa")%>% dplyr::group_by(Kebele) %>% dplyr::summarise(No_of_kebele= n()) %>%  dplyr::arrange(No_of_kebele)

Dewat_kebeles$Kebele<- factor(Dewat_kebeles$Kebele, levels=Dewat_kebeles[["Kebele"]])

#plot fig 4

fig5 <- Dewat_kebeles %>% plot_ly()
fig5 <- fig5 %>% add_trace(x = ~Kebele, y = ~No_of_kebele, type = 'bar',  name = '', marker = list(color = '#FFBF00'))
...
```{r}
Artuma_kebeles<- hh_data %>% dplyr::select('Woreda', 'Kebele') %>% drop_na()%>%  dplyr::filter(Woreda=="Artuma Fursi")%>% dplyr::group_by(Kebele) %>% dplyr::summarise(No_of_kebele= n()) %>% dplyr::arrange(No_of_kebele)

Artuma_kebeles$Kebele<- factor(Artuma_kebeles$Kebele, levels=Artuma_kebeles[["Kebele"]])

#plot fig 4

fig5 <- Artuma_kebeles %>% plot_ly()
fig5 <- fig5 %>% add_trace(x = ~Kebele, y = ~No_of_kebele, type = 'bar',  name = '', marker = list(color = '#2B3A55'))
...
### LIBO KEMKEM WOREDA BY KEBELE {data-width=500}

```{r}
Libo_kebeles<- hh_data %>% dplyr::select('Woreda', 'Kebele') %>% drop_na()%>%  dplyr::filter(Woreda=="Libo Kemkem")%>% dplyr::group_by(Kebele) %>% dplyr::summarise(No_of_kebele= n()) %>% dplyr::arrange(No_of_kebele)

Libo_kebeles$Kebele<- factor(Libo_kebeles$Kebele, levels=Libo_kebeles[["Kebele"]])


fig5 <- Libo_kebeles %>% plot_ly()
fig5 <- fig5 %>% add_trace(x = ~Kebele, y = ~No_of_kebele, type = 'bar',  name = '', marker = list(color = '#FFBF00'))

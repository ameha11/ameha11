---
title: "PSNP 5 SPOT-CHECK DASHBOARD"
output: 
...
woreda_data<-read_dta("C:/Survey Spotcheck II/Data/Raw_data/PSNP_WOREDA_FINAL_1/PSNP_WOREDA_FINAL_1.dta")
...
#Kebele data
kebele_data<-read_dta("C:/Survey Spotcheck II/Data/Raw_data/PSNP_5_KEBELE_FINAL/PSNP_5_KEBELE_FINAL.dta")
...
#Household data
hh_data<-read_dta("C:/Survey Spotcheck II/Data/Raw_data/PSNP_5_HOUSEHOLD_FINAL/PSNP_5_HOUSEHOLD_FINAL.dta")
...
### **Woreda Offices** {data-width=300}

```{r}

#Wored Tool Type
Woreda_tools <- c("WFSD","WEWRD","WNRD",	"WLS","WOLSA","WCPO","WOFED","FDP","WEPSC","WGHO")
Woreda_tools_no <-c(WFSD,	WEWRD,	WNRD,	WLS,	WOLSA,	WCPO,	WOFED,	FDP,	WEPSC,	WGHO)

Woreda_offices <- data.frame(Woreda_tools, Woreda_tools_no)
Woreda_offices <- Woreda_offices %>% arrange(Woreda_tools_no)
Woreda_offices$Woreda_tools<- factor(Woreda_offices$Woreda_tools, levels=Woreda_offices[["Woreda_tools"]])


#plot fig
fig1 <- Woreda_offices %>% plot_ly()
fig1 <- fig1 %>% add_trace(x = ~Woreda_tools, y = ~Woreda_tools_no, type = 'bar',  name = 'Woreda Offices', marker = list(color = '#5D3891'))
...
### **Meetings in last quarter** {data-width=300}
```{r}


Meetings<-woreda_data %>% 
  plot_ly(x=~region, 
          y=~P1_B2, 
          color=~woreda,
          colors="Set1",
          ...
          text=~P1_B2,
          textposition="auto",
          hovertext = ~paste("Region=", region,"<br>", "Woreda=", woreda, "<br>", "Meetings=", P1_B2, "<br>"), 
          hoverinfo = "text") %>% 
  add_bars() %>% 
  layout(title="",
...
Meetings <- plotly::config(Meetings, displayModeBar = FALSE)
...
### **Available Documents** {data-width=400}  
```{r }

doc_p1<-read_dta("C:/Survey Spotcheck II/Data/Raw_data/PSNP_WOREDA_FINAL_1/documents_1.dta")
...
### **Record keeping** {data-width=400}
```{r}
colors4=c('#1F8A70','#30E3DF','#497174')
record_k1<-read_dta("C:/Survey Spotcheck II/Data/Raw_data/PSNP_WOREDA_FINAL_1/Record_Keeping_1.dta")
...
p1_record <- record_k1 %>%
  select(region, Record_Keeping_1__id, region, woreda, P1_B21, P1_B21_1) %>% 
...
### **Recork keeping Type** {data-width=400}
```{r}
record_k2<-read_dta("C:/Survey Spotcheck II/Data/Raw_data/PSNP_WOREDA_FINAL_1/Record_Keeping_1.dta")
...
p2_record <- record_k2 %>%
  select(region, Record_Keeping_1__id, P1_B21, P1_B21_1) %>% drop_na() %>% 
  mutate(Record_Keeping_1__id = str_replace(Record_Keeping_1__id,"PDS monthly payroll", "PDS payroll"), Record_Keeping_1__id = str_replace(Record_Keeping_1__id,"PW attendance sheet", "PW attendance")) %>% 
  #drop_na() %>% 
  group_by(region,P1_B21_1) %>%
...
appeal<-woreda_data %>% select(region,P1_F5) %>%drop_na() %>% 
  group_by(region) %>% summarise(n_answer=sum(P1_F5))
  
colors=c('#F99417','#5D3891','#89375F','#CE5959')
p_appeal <- plot_ly(appeal, labels = ~region, values = ~n_answer, type = 'pie', marker = list(colors = colors,
...
### **Document Availablity** {data-width=400}

```{r }
k_doc1<-read_dta("C:/Survey Spotcheck II/Data/Raw_data/PSNP_5_KEBELE_FINAL/PSNP_available_doc.dta")
...
p_doc_aval<-k_doc_aval %>% 
  plot_ly(x=~PSNP_available_doc__id, 
          y=~count, 
...
p_doc_aval
```
### **Re-targeted Condacted** {data-width=300}
```{r}

K_retargeting <- kebele_data %>%
  select(region, woreda, C_Q10) %>% 
  mutate(C_Q10 = str_replace(C_Q10,"Yes, saw documentary evidence", "Yes")) %>% 
  drop_na() %>% 
  group_by(region,C_Q10) %>%
  count(name="n_answers") %>% 
  group_by(region) %>% 
  mutate(percent_answers = n_answers / sum(n_answers)) %>% 
  mutate(percent_answers_label = percent(percent_answers, accuracy = 1)) 
...
p_k_retargeting <- plotly::config(p_k_retargeting, displayModeBar = FALSE)
...
### **Days each person work on public works per week** {data-width=300}
```{r }
pw_day_per_week<-kebele_data %>% 
  group_by(region, interview__key) %>% 
...
p_day_per_week <-avg_day_per_week_reg %>% 
  plot_ly(x=~region, 
          ...
          textposition="outside",
          hovertext = ~paste("Region=", region, "<br>", "Avg PW days=", avg_day_per_week, "<br>"), 
          hoverinfo = "text") %>% 
  add_bars() %>% 
  layout(title="",
         xaxis = list(title="Region"),
...
### **Hours each person work on public works per week(HR)** {data-width=300}
```{r }
...
p_hr_per_week <-avg_hr_per_week_reg %>% 
  plot_ly(x=~region, 
          ...
          textposition="outside",
          hovertext = ~paste("Region=", region, "<br>", "Avg. PW hour=", avg_hr_per_week, "<br>"), 
          hoverinfo = "text") %>% 
  add_bars() %>% 
  layout(title="",
         xaxis = list(title="Region"),
...
proj_reg<-projects %>% select(region,Impl_proj, Plan_proj) %>% 
  group_by(region) %>% summarise(impl=sum(Impl_proj), plan=sum(Plan_proj))


p_proj <- proj_reg %>% plot_ly()
p_proj <- p_proj %>% add_trace(x = ~region, y = ~impl, type = 'bar',  name = 'Implmented', marker = list(color = '#FFBF00'))
p_proj <- p_proj %>% add_trace(x = ~region, y = ~plan, type = 'bar', name = 'Planned', marker = list(color = '#5D3891'))
...
#Excluded <-hh_data %>% filter(Id_8__6==1) %>% nrow()
```
...
fig1 <- fig1 %>% layout(xaxis = list(title = 'HH category'), barmode = 'group')          
fig1 <- fig1 %>% layout(yaxis = list(title = 'n'), barmode = 'group')
...
targeting<- hh_data %>% 
  ...
  filter(S4_Q2=="Yes") %>% 
   ...
   plot_ly(x=~region, 
          ...
          colors="Set1",
          color=~region,
          hovertext = ~paste("Region=", region, "<br>", "Percent=", percent_answers_label, "<br>", "n=",n_answers), 
          hoverinfo = "text") %>% 
...
column 
-------------------------------------
### **Average household land size per Region** 
...
p_land <-avg_land_per_hh_reg %>% 
  plot_ly(x=~region, 
          ...
          textposition="outside",
          hovertext = ~paste("Region=", region, "<br>", "Avg Land Size=", avg_land_size_hh, "<br>"), 
          hoverinfo = "text") %>% 
  add_bars() %>% 
  layout(title="",
         xaxis = list(title="Region"),
...
### **Lump sum transfers(%)**
```{r }
colors3=c('#009FBD','#FF6000')
Lump_sum<-hh_data %>% select(region, S5_Q44) %>% drop_na() %>% 
  group_by(region, S5_Q44) %>%
  count(name="n_answers") %>%
  group_by(region) %>% 
  mutate(percent_answers = (n_answers / sum(n_answers))) %>% 
  #mutate(across(where(is.numeric), round, 0)) %>% 
  ...
  layout(title="",
         ...
         yaxis = list(title = "%"),
         barmode="group")
Lump_sum <- plotly::config(Lump_sum, displayModeBar = FALSE)
Lump_sum
```
...
```{r }
colors2=c('#009FBD','#FF6000')
lh_suport<-hh_data %>% 
  group_by(region, S8_Q1) %>%
  count(name="n_answers") %>%
  group_by(region) %>% 
  mutate(percent_answers = (n_answers / sum(n_answers))) %>% 
  mutate(percent_answers_label = percent(percent_answers, accuracy = 1)) %>%
...
### **How did you receive your last transfer**
```{r}

...
### **Avg no. of Technical Staff** {data-width=300}  
```{r }
colors6=c('#009FBD', '#FFD93D', '#FF6000')
avg_tech_staff<-woreda_data %>% 
  group_by(region, interview__key) %>% 
  summarise(Average_staff = mean(P1_B20))%>% 
  drop_na() 

avg_tech_staff_reg<-avg_tech_staff %>% 
  group_by(region) %>%
  summarise(avg_tech_staff = mean(Average_staff)) %>% 
  mutate(across(where(is.numeric), round, 0))

p_avg_tech_staff <-avg_tech_staff_reg %>% 
  plot_ly(x=~region, 
          y=~avg_tech_staff, 
          color=~region,
          colors=colors6,
          text=~avg_tech_staff,
          textposition="outside",
          hovertext = ~paste("Region=", region, "<br>", "Avg. PW hour=", avg_tech_staff, "<br>"), 
          hoverinfo = "text") %>% 
  add_bars() %>% 
  layout(title="",
         xaxis = list(title=""),
         yaxis = list(title = "no"),
         barmode="group")
p_avg_tech_staff <- plotly::config(p_avg_tech_staff, displayModeBar = FALSE)
p_avg_tech_staff
```
...
```{r}
colors4=c('#1F8A70','#30E3DF','#497174', '#009FBD','#FF6000', '#FFD93D', '#7AA874')
...
### **Forms Record Maintained** {data-width=400}
```{r}
colors55=c('#30E3DF', '#FFD93D','#FF6000', '#009FBD')
...
### **Re-targeting Conducted** {data-width=300}
```{r}
colors42=c('#FF6000', '#009FBD')
...
record_k2<-read_dta("C:/Survey Spotcheck II/Data/Raw_data/PSNP_5_KEBELE_FINAL/records.dta")
record_k2 <- as_factor(record_k2) 
record_k2 <- droplevels(record_k2)
options(digits=1) 

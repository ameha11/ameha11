...
### BASELINE DATA BY ENUMERATOR {data-width=400}
...
### BASELINE DATA BY KEBELE {data-width=400}
```{r}
...
animal_service<-hh_endline_mrg %>% select(D15) %>% group_by(D15) %>% 
  ...
  hc_title(text = 'ACCESS TO ANIMAL HEALTH SERVICES', style = list(fontSize = '15px', fontWeight = 'bold')) 
animal_service
...
water_acces<-hh_endline_mrg %>% select(D48) %>% group_by(D48) %>% 
  ...
  hc_tooltip(pointFormat='<b>Proportion: </b> {point.percentage:,.2f}%') %>% 
  hc_title(text = 'MAIN SOURCE OF DRINKING WATER', style = list(fontSize = '15px', fontWeight = 'bold')) 
water_acces

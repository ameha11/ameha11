...
Besu_pro <- employee %>% filter(supervisor=="Besufikad Birhanu") %>% nrow()
Habtamu_pro <- employee %>% filter(supervisor=="Habtamu Ayele") %>% nrow()
Sisay_pro <- employee %>% filter(supervisor=="Sisay addis") %>% nrow()
```
...
```{r}
sup_name <- c("Asnake", "Addis", "Selamu", "Solomon", "Natnael", "Hiwot", "Besufikad", "Habtamu", "Sisay")
sup_emp <- c(Asnake_emp, Addis_emp, Selamu_emp, Solomon_emp, Natnael_emp, Hiwot_emp, Besu_emp, Habtamu_emp, Sisay_emp)
sup_pro <- c(Asnake_pro, Addis_pro, Selamu_pro, Solomon_pro, Natnael_pro, Hiwot_pro, Besu_pro, Habtamu_pro, Sisay_pro)
sup_cli <- c(Asnake_cli, Addis_cli, Selamu_cli, Solomon_cli, Natnael_cli, Hiwot_cli, Besu_cli, Habtamu_cli, Sisay_cli)

data_sup <- data.frame(sup_name, sup_emp, sup_pro, sup_cli)
data_sup <- data_sup %>% arrange(sup_cli)
#to arrange
...
```{r expected}
# Expected data
supervisor <- c("Addis Mengistu",	"Asnake Getu",	"Hiwot Zelalem",	"Natnael Shewanes",	"Selamu Bulado","Solomon Genore")
emp_fed_exp <- c(0,	0, 62.5, 62.5,	0,	0)
emp_reg_exp <- c(0,	0, 52.5, 52.5, 210, 105)
emp_zone_exp <- c(80,	0, 100, 100, 80, 120)
emp_woreda_exp <- c(160, 160, 80, 80, 224, 256)
client_exp <- c(240, 240, 120, 120, 336, 384)
profile_exp <- c(56, 40, 63, 63, 114,109)

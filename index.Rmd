...
```{r}
sup_name <- c("Asnake", "Addis", "Selamu", "Solomon", "Natnael", "Hiwot", "Besufikad", "Habtamu", "Sisay", "Bekalu", "Henok")
sup_emp <- c(Asnake_emp, Addis_emp, Selamu_emp, Solomon_emp, Natnael_emp, Hiwot_emp, Besu_emp, Habtamu_emp, Sisay_emp, Bekalu_emp, Henok_emp)
sup_pro <- c(Asnake_pro, Addis_pro, Selamu_pro, Solomon_pro, Natnael_pro, Hiwot_pro, Besu_pro, Habtamu_pro, Sisay_pro, Bekalu_pro, Henok_pro)
sup_cli <- c(Asnake_cli, Addis_cli, Selamu_cli, Solomon_cli, Natnael_cli, Hiwot_cli, Besu_cli, Habtamu_cli, Sisay_cli, Bekalu_cli, Henok_cli)
...
```{r expected}
# Expected data
supervisor <- c("Addis Mengistu",	"Asnake Getu",	"Hiwot Zelalem",	"Natnael Shewanes",	"Selamu Bulado","Solomon Genore", "Besufikad", "Habtamu", "Sisay", "Bekalu", "Henok")
emp_fed_exp <- c(0,	0, 62.5, 62.5,	0,	0, 0, 0, 0, 0, 0)
emp_reg_exp <- c(0,	0, 52.5, 52.5, 210, 105, 105, 105, 105, 105, 315)
emp_zone_exp <- c(80,	0, 100, 100, 80, 120, 80, 120, 80, 80, 80)
emp_woreda_exp <- c(160, 160, 80, 80, 224, 256, 224, 192,192, 192, 224)
client_exp <- c(240, 240, 120, 120, 336, 384, 336, 288, 288, 288, 336)
profile_exp <- c(56, 40, 63, 63, 114,109, 93, 93, 85, 85, 199)
...
emp_fed_act <- employee %>% filter(org_level == "Federal") %>% 
  group_by(supervisor) %>% summarise(count_fed_emp = n()) %>%
  arrange(count_fed_emp)

emp_reg_act <- employee %>% filter(org_level == "Regional") %>% 
  group_by(supervisor) %>% summarise(count_reg_emp = n()) %>% 
  arrange(count_reg_emp)

emp_zone_act <- employee %>% filter(org_level == "Zonal") %>% 
  group_by(supervisor) %>% summarise(count_zone_emp = n()) %>% 
  arrange(count_zone_emp)

emp_woreda_act <- employee %>% filter(org_level == "Woreda") %>% 
  group_by(supervisor) %>% summarise(count_woreda_emp = n()) %>% 
  arrange(count_woreda_emp)

# Actual data client
client_act <- client %>% group_by(supervisor) %>% summarise(count_client = n()) %>% arrange(count_client)

# Actual data profile
profile_act <- profile %>% group_by(supervisor) %>% summarise(count_profile = n()) %>% arrange(count_profile)
